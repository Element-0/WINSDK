...
public:
    ...
    template<typename _InterfaceType,
        ::std::enable_if_t<!::std::disjunction_v<
            ...
            ::std::is_same<_InterfaceType, _com_ptr_t>, // call _com_ptr_t* p
            ::std::is_same<_InterfaceType, char>, // call PCSTR str, IUnknown* pOuter, DWORD dwClsContext
            ::std::is_same<_InterfaceType, const char>,
            ::std::is_same<_InterfaceType, wchar_t>, // call LPCWSTR str, IUnknown* pOuter, DWORD dwClsContext
            ::std::is_same<_InterfaceType, const wchar_t>
        >, int> = 0>
    ...
    template<typename _OtherIID> // also _IIID
    bool operator!=(const _com_ptr_t<_OtherIID>& p) const
    {
        return !(operator==(p));
    }

    template<typename _InterfaceType> // also Interface
    bool operator!=(_InterfaceType* p) const
    {
        return !(operator==(p));
    }
...
//
template<typename _InterfaceType> bool operator==(decltype(nullptr), const _com_ptr_t<_InterfaceType>& p) noexcept
{
    return p == nullptr;
}
...
template<typename _Interface> bool operator!=(decltype(nullptr), const _com_ptr_t<_Interface>& p) noexcept
{
    return p != nullptr;
}
